...
from webat.model.base_tracker import *
import csv
...
from webat.utility.utils import *
from webat.utility.utils_ml import prediction
...
class ObjectTracker(EuclideanDistTracker):      # Takes the bounding box of the objct and save them into one array
    ...
    def run_detection_models(self, image_of_detected_object_roi):
        ...
        if binary_pred[0][0] < 0.9:         # Skip if the probability is below the threshold (0.5 => 0.9)
            return 'empty', None
        ...
        if multiclass_pred[0][0] > self.thresholds['bat']:
            ...
            object_arg = np.argmax(multiclass_pred[0][1:]) + 1
        
        if object_arg == 1 and multiclass_pred[0][1] < self.thresholds['bird']:     # Remove false postive birds
            object_arg = 2
        
        if object_arg == 3 and multiclass_pred[0][3] < self.thresholds['insect']:     # Remove false postive insects
            object_arg = 2
    ...
    def handle_frame(self, frame, contours, frame_num):
        ...
        for cnt in contours:
            ...
            if area > 40:                   # remove too small contours to identify
                ...
                cv2.putText(frame, f'{object_type} {object_id}', (cX, cY+35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.colors[object_type], 2)
    ...
    def save_tracking_results(self, bounding_box, object_id, object_type, frame_num, multiclass_pred, cX, cY, cnt, area, new_object_directory_name):
        ...
        # time_offset = {'test_Middle': 0, 'test_Far_Right': 0, 'Far_Left': 0}    # Delay occurs
